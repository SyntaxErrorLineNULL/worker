...
type Task struct {
	...
	// It helps in identifying and tracking the job in logs.
	name string
	...
	// It's an optional channel, primarily used for coordinating job completion with other processes.
	doneCh chan<- struct{}
...
func NewTask(timeout time.Duration, name string, processing worker.Processing, processingInput interface{}) *Task {
	return &Task{
		timeout:         timeout,
		err:             make(chan error),
		name:            name,
...
// SetWaitGroup assigns the provided WaitGroup to the task's worker WaitGroup field.
...
func (t *Task) SetWaitGroup(wg *sync.WaitGroup) error {
	...
	// Assign the provided non-nil WaitGroup to the task's worker WaitGroup field.
	t.wg = wg
...
//
// Note: the channel must be closed after you receive a signal that the currentProcess is complete.
...
func (t *Task) GetError() <-chan error {
	return t.err
}

// String returns the name of the job.
// This method retrieves the name assigned to the job instance.
func (t *Task) String() string {
	return t.name
}

// Run executes the job, running the main function within a context with a timeout.
// It handles potential panics, signals job completion, and manages timeouts.
func (t *Task) Run(workerTimeout time.Duration) {
	// Defer cleanup steps.
	defer func() {
		...
		// Decrement the wait group worker WaitGroup counter, if it is available.
	...
	// If the timeout is less than the worker's timeout, use the worker's timeout.
	if t.timeout < workerTimeout {
		t.timeout = workerTimeout
	}
