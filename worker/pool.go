...
// NewPool creates a new instance of a worker pool with the specified options.
// It initializes the pool, sets up the context, and prepares the pool for managing workers and tasks.
func NewPool(options *worker.Options) *Pool {
...
// each of which is responsible for processing jobs from a shared queue. This method also continuously listens for stop signals
...
func (p *Pool) loop() {
	...
	defer func() {
		...
		if rec := recover(); rec != nil {
			...
			if err := worker.GetRecoverError(rec); err != nil {
				p.logger.Printf("Worker pool encountered a error: %v", err)
			}
		}
	...
	for {
		select {
		// Why such a case is needed, at some point the stopCh worker closed, a negative waitGroup was triggered,
		// and there was a leak. It may happen that all workers will fall and then the task or tasks,
		// depending on what channel you use, will just wait for someone to pick them up.
		// A crash doesn't mean the end, a crash means we have to rebuild the worker and continue processing.
		case workerError, ok := <-p.workerErrorCh:
			if !ok {
				// Everything's fine, they just closed the channel,
				// which means the processing is basically complete.
				p.logger.Print("worker error channel is close")
				break
			}
			...
			if !p.stopped {
				...
				if p.maxRetryWorkerRestart != workerError.Instance.GetRetry() {
					...
					// This ensures that the main routine waits for the worker restart currentProcess to complete.
					p.workerWg.Add(1)

					// Start a new goroutine to restart the worker.
					// The worker restart currentProcess will be handled asynchronously.
					workerError.Instance.Restart(p.workerWg)
				} else {
		...
		case <-p.stopCh:
			...
			p.logger.Print("worker pool stop")
			return
...
func (p *Pool) AddTaskInQueue(task worker.Task) (err error) {
	...
	defer func() {
		if rec := recover(); rec != nil {
			...
			if err != nil {
				p.logger.Printf("panic encountered while adding task to queue: %v", err)
				return
			}
		}
	}()
	...
	if p.taskQueue == nil {
		p.logger.Print("task queue is nil")
	...
	case <-p.taskQueue:
		...
		p.logger.Print("task queue is close")
	...
	if err := task.SetContext(p.ctx); err != nil {
		p.logger.Print("failed to set parent context for task")
		return err
	}
...
// The wait group (workerWg) is also incremented to track the new worker's lifecycle.
// The method returns an error if the worker count cannot be incremented
func (p *Pool) AddWorker(wr worker.Worker) (err error) {
	...
	defer func() {
		if rec := recover(); rec != nil {
			...
			if err = worker.GetRecoverError(rec); err != nil {
				// If an error is successfully created from the panic, return immediately.
				// This prevents further execution in case of a critical failure.
				p.logger.Printf("panic occurred while adding worker: %v", err)
			}
		}
	}()
	...
	if err = wr.SetQueue(p.taskQueue); err != nil {
		p.logger.Print("failed to set worker queue")
		return err
	}
	...
	if err = wr.SetWorkerErrChannel(p.workerErrorCh); err != nil {
		p.logger.Print("failed to set worker error channel")
		return err
	}
...
func (p *Pool) Stop() {
	// Defer a function to recover from any panics that occur during the shutdown currentProcess.
	...
	defer func() {
		if rec := recover(); rec != nil {
			...
			if err := worker.GetRecoverError(rec); err != nil {
				// If an error is successfully created from the panic, return immediately.
				// This prevents further execution in case of a critical failure.
				p.logger.Printf("panic occurred during pool shutdown: %v", err)
			}
		}
	}()

	// Use sync.Once to ensure that the shutdown currentProcess is only executed once.
	// This prevents the pool from being shut down multiple times, which could lead to errors.
	p.onceStop.Do(func() {
		// Lock the mutex to ensure that the shutdown currentProcess is thread-safe.
		// This prevents other goroutines from interfering with the shutdown currentProcess.
		p.mutex.Lock()
		...
		case <-p.workersShutdown():
			p.logger.Print("all workers have been successfully shut down")
		default:
		...
		// Send a signal to the stopCh channel to indicate that the stop currentProcess has started.
		...
		// This also signals to any listeners that the stop currentProcess is complete.
		close(p.stopCh)
	})
}
	...
	defer func() {
		...
		if r := recover(); r != nil {
			...
			if err := worker.GetRecoverError(r); err != nil {
				// If an error is successfully created from the panic, return immediately.
				// This prevents further execution in case of a critical failure.
				p.logger.Printf("error during worker shutdown: %v", err)
			}
		}
	}()
...
// workersShutdown handles the shutdown currentProcess for all workers in the pool.
...
func (p *Pool) workersShutdown() <-chan struct{} {
	...
	defer func() {
		...
		if r := recover(); r != nil {
			...
			if err := worker.GetRecoverError(r); err != nil {
				// If an error is successfully created from the panic, return immediately.
				// This prevents further execution in case of a critical failure.
				p.logger.Printf("workersShutdown panic: %v", err)
			}
			return
		}
	}()

	// Create a channel to signal the completion of the shutdown currentProcess.
	...
	for _, wr := range p.workers {
		// Stop each worker that is not already stopped.
		if !wr.IsStop() {
			// If the worker is active, initiate its shutdown currentProcess by calling its Stop method.
	...
	// Wait for all workers to finish their shutdown currentProcess.
	...
	// Send a signal on the done channel to indicate that the shutdown currentProcess is complete.
